...
class Data:
    ...
    def get_batch(self, batch_size: int, only_once: bool = False) -> Tuple[np.ndarray, np.ndarray]:
        ...
        while len(used_image_ids) < batch_size:
            if self.idx + batch_size < self.size:
                used_image_ids += self.image_ids[self.idx : self.idx + batch_size]
                used_captions += self.captions[self.idx : self.idx + batch_size]
                self.idx += batch_size
            else:
