...
from model.Lenet.main import Lenet


def eval_accuracy(model: Lenet, data: MnistData, batch_size: int) -> float:
    if data.size == 0:
...
class MnistTrainer:
    def __init__(self, model: Lenet, optimizer: BaseOptimizer, data: MnistData) -> None:
        self.model = model
